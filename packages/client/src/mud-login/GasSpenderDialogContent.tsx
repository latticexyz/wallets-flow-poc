...
export function GasSpenderDialogContent() {
  ...
  const [result, registerSpender] = useCreatePromise(async () => {
  ...
  return (
    <Dialog.Content>
      ...
      {result.status === "rejected" ? <>Error: {String(result.reason)}</> : null}
      ...
      <Flex gap="3" mt="4" justify="end">
        ...
        <Button loading={result.status === "pending"} onClick={registerSpender}>
          Set up spender
        </Button>
      </Flex>
    </Dialog.Content>
